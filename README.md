...
> [!NOTE]
> **v5.7.1** — fixes a use-after-free in the vendored AsyncTCP library's connection-abort path (upstream [ESP32Async/AsyncTCP#118](https://github.com/ESP32Async/AsyncTCP/pull/118)) that this firmware's web admission policy hit directly: rejecting a connection under concurrent HTTP/SSE load could free an `AsyncClient` while a queued async-TCP event still referenced it, crashing the node (`oom_gate` reset, panic, or a watchdog reset, depending on timing). The bundled AsyncTCP is bumped from the pre-fix v3.4.10 to v3.5.0. Also trims several temporary allocations out of `/api/health` (fewer `String` copies, raw-JSON `reset_history` instead of double-escaped), recalibrates the web admission heap floor against the node's real ~32 kB steady-state free heap, and adds an on-device CSI/ML feedback loop (`POST /api/csi/feedback`, paginated export) for false-alarm labeling. Verified with a hardware MQTT alarm-router test, ~3.6 h of bounded HTTP/SSE load across five concurrency/SSE combinations, and a clean 24-hour quiet soak (1434 samples, zero reboots) on the fixed build — parity with the 5.7.0 validation bar. See [CHANGELOG](CHANGELOG.md#571---2026-09-08).

> [!TIP]
> **v5.7.0** — reliability release, driven by a multi-day field soak. Three defects that only a long run exposes: (1) every heap decision read `MALLOC_CAP_INTERNAL`, which also counts a leftover IRAM-only region that `malloc()` / `operator new` can never obtain — the figures ran ~41 kB high, so the dev7 **web low-heap gate could never close** (`close_count: 0` across nine OOM deaths); all heap now reads the byte-addressable `MALLOC_CAP_8BIT`, with both capabilities reported side by side in `/api/health` → `heap{}`. (2) A single **sleeping dashboard tab held ~48 kB** in its SSE queue — `AsyncEventSource` bounds that queue by message *count*, and the `SSE_MAX_QUEUED_MESSAGES=8` override was a dead define (PlatformIO `build_flags` **replace** an inherited list instead of extending it); now bounded, with a send-site backlog gate and `sse{}` counters. (3) **MQTT was leaving over the weak CSI WiFi radio instead of Ethernet** — on a flat LAN the WiFi STA outranks Ethernet on lwIP `route_prio` and silently takes the default route, so alarm delivery shared the fate of the sensor radio; the new `EthDefaultNetifPolicy` re-asserts Ethernet periodically and forces an MQTT reconnect only on a genuine routing change, never on a link flap (`/api/health` → `ethernet.route{}`). Plus self-reported outage alerts after a dirty boot, a selectable P95/P99 adaptive CSI percentile, an Ethernet flap-rate counter, CSI starvation gating and runtime webhook provisioning (`/api/notifications/config`). Validated on a seven-day field soak (177 h, zero reboots); 376/376 native tests, all five shipped builds green. See [CHANGELOG](CHANGELOG.md#570---2026-08-31).
...
| POST | `/api/csi/feedback` | **T9** Label the most recent ML inference (`?label=motion\|no_motion`) for false-alarm correction. Rejects `400` if no inference has run yet, `409` if the snapshot is older than 5 s, `503` if the store is busy/unavailable |
| GET | `/api/csi/feedback/export` | **T9** Paginated export of labeled samples for offline retrain (`?limit=8&after_seq=N`, capped at 8/page) — Basic auth. Returns schema/feature-count metadata and `available`/`read_error` flags; a corrupted store is never silently skipped |
...
| v5.7.0 | **Reliability release, driven by a multi-day field soak.** Heap decisions read the inflated `MALLOC_CAP_INTERNAL` capability instead of the byte-addressable `MALLOC_CAP_8BIT`, so the web low-heap gate never closed (`close_count: 0` across nine field OOM deaths) — both capabilities now reported side by side in `/api/health` → `heap{}`. A sleeping dashboard tab could hold ~48 kB in its SSE queue (a dead `SSE_MAX_QUEUED_MESSAGES` override, since PlatformIO `build_flags` replace rather than extend an inherited list) — now bounded, with a backlog gate and `sse{}` counters. MQTT could silently leave over the weak CSI WiFi radio instead of Ethernet on a flat LAN; `EthDefaultNetifPolicy` re-asserts Ethernet and reconnects MQTT only on a genuine routing change. Plus dirty-boot outage alerts, a selectable P95/P99 adaptive CSI percentile, an Ethernet flap-rate counter, CSI starvation gating, and runtime webhook provisioning. Seven-day field soak (177 h, zero reboots); 376/376 native tests, all five shipped builds green. |
| v5.7.1 | **AsyncTCP use-after-free fix.** Rejecting a connection under concurrent HTTP/SSE load could free an `AsyncClient` object while a queued async-TCP event still referenced it, crashing the node — traced to a bug already fixed upstream (`ESP32Async/AsyncTCP#118`) months after this project's pinned version; bumped v3.4.10 → v3.5.0. Also trims temporary allocations out of `/api/health`, recalibrates the web admission heap floor against the node's real steady-state free heap, and adds an on-device CSI/ML feedback loop (`POST /api/csi/feedback`, paginated `GET /api/csi/feedback/export`) for false-alarm labeling and offline retrain. Hardware MQTT alarm-router test, bounded HTTP/SSE load across five concurrency/SSE combinations, and a clean 24-hour quiet soak (1434 samples, zero reboots); 412/412 native tests, all five shipped builds green. |
---
